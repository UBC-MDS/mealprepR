testthat::test_that("find_fruits_veg has the following behaviour:
     ...
     5. If the find_fruits_veg will handle wrong input", {
  ...
  testthat::expect_equal(find_fruits_veg(df2, 'num'), 
                         'It is a empty data frame or too many missing data')
  testthat::expect_equal(find_fruits_veg(df1, 'xxx'), 
                         "Wrong input of type_of_out, try 'num' or 'categ'")
})
